<x-app-layout>
    <x-slot name="title">{{ __('world::general.update_state') }}: {{ $state->name }}</x-slot>

    <x-heading>
        <x-card.title>{{ __('world::general.update_state') }}: {{ $state->name }}</x-card.title>
    </x-heading>
                        ...
                        <x-slot name="input">
                            <x-select id="country_id" class="tom-select" required>
                                @foreach($countries as $country)
                                    <option value="{{ $country->id }}" @selected(old('country_id') ?? $state->country->id === $country->id)>
                                        {{ $country->name }}
                                    </option>
                                @endforeach
                            </x-select>
